    ...
    def analyze_papers(self, papers: List[Dict], drug_name: Optional[str] = None) -> pd.DataFrame:
        ...
        for paper in papers:
            try:
                ...
                # If we found ages, prefer the one that appears most often or is in a more reliable range
                if found_ages:
                    ...
                    # Prefer ages that appear more often
                    max_count = max(age_counts.values())
                    most_common_ages = [age for age, count in age_counts.items() if count == max_count]
                    
                    # If multiple ages appear equally often, prefer the one in elderly range (65-95)
                    elderly_ages = [age for age in most_common_ages if 65 <= age <= 95]
                    if elderly_ages:
                        case['age'] = elderly_ages[0]
                    else:
                        case['age'] = most_common_ages[0]
